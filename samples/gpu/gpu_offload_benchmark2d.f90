...
!$omp target teams distribute parallel do collapse(2) map(tofrom: A(0:m-1,j1:j2)) device(dev)
...
!$omp target teams distribute parallel do collapse(2) map(tofrom: A(0:m-1,0:n-1))
  do j=0,n-1;do i=0,m-1
     iter=0
     do while (iter < iter_max)
        A(i,j) = A(i,j)*(A(i,j)-1.0)
...
subroutine benchmark2d_omp_gpu_ij(nthread, iter_max, m, n, A)
  integer, intent(in) :: nthread, iter_max, m, n
  real*8, dimension(0:m-1,0:n-1), intent(inout) :: A
  integer :: iter
...
!$omp target parallel do collapse(2) map(tofrom: A(0:m-1,0:n-1),iter)
  do i=0,n-1;do j=0,m-1
     ...
     enddo
   enddo; enddo
end subroutine benchmark2d_omp_gpu_ij
...
subroutine benchmark2d_docon_2dev_hack(nthread, iter_max, m, n, A)
  integer, intent(in) :: nthread, iter_max, m, n
  real*8, dimension(0:m-1,0:n-1), intent(inout) :: A
  integer :: iter,dev
!$omp parallel do private(dev,j1,j2,iter,i,j) shared(A,iter_max,m,n)
...
!$ACC set device_num(dev)
     ...
     enddo
  enddo
end subroutine benchmark2d_docon_2dev_hack

subroutine benchmark2d_acc(nthread, iter_max, m, n, A)
  integer, intent(in) :: nthread, iter_max, m, n
  real*8, dimension(0:m-1,0:n-1), intent(inout) :: A
  integer :: iter
...
!$ACC parallel loop !!collapse(2) cause slow down
  ...
  enddo; enddo
!ACC end data
end subroutine benchmark2d_acc

subroutine benchmark2d_acc_2dev(nthread, iter_max, m, n, A)
  integer, intent(in) :: nthread, iter_max, m, n
  real*8, dimension(0:m-1,0:n-1), intent(inout) :: A
  integer :: iter,dev
...
!$ACC parallel loop !!collapse(2) cause slow down
  ...
  enddo
!ACC end data
end subroutine benchmark2d_acc_2dev
...
!$   run_time = omp_get_wtime() 
      ...
      if(input == -4) then
         call benchmark2d_docon(1,iter_max,m,n,A2)
         subname='benchmark2d_docon'
      endif
      if(input == -42) then
         call benchmark2d_docon_2dev_hack(1,iter_max,m,n,A2)
         subname='benchmark2d_docon_2dev_hack'
      endif
      if(input == -5) then
         call benchmark2d_acc(1,iter_max,m,n,A2)
         subname='benchmark2d_acc'
      endif
      if(input == -52) then
         call benchmark2d_acc_2dev(1,iter_max,m,n,A2)
         subname='benchmark2d_acc_2dev'
      endif
