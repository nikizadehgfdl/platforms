...
compiler_options='-Duse_PYTHON -DUSE_FMS2_IO -DMAX_FIELDS_=100 -DNOT_SET_AFFINITY -D_USE_MOM6_DIAG -D_USE_GENERIC_TRACER  -DUSE_PRECISION=2 -D_USE_LEGACY_LAND_ -Duse_AM3_physics'
linker_options=''
if [[ "$target" =~ "stdpar" ]] ; then 
    compiler_options="$compiler_options -stdpar -gpu=managed -Minfo=accel"
    linker_options="$linker_options -stdpar "
fi
if [[ "$target" =~ "accgpu" ]] ; then 
    compiler_options="$compiler_options -acc -gpu=managed  -Minfo=accel"
fi
