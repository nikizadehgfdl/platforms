...
#INCLUDE += $(shell pkg-config --cflags-only-I netcdf-fortran)
#INCLUDE += $(shell pkg-config --cflags-only-I mpich)
...
FFLAGS_OPT = -O3 -debug minimal -fp-model source -qoverride-limits -qopt-prefetch=3 $(ISA_OPT)
FFLAGS_REPRO = -O2 -debug minimal -fp-model source -qoverride-limits -g -traceback $(ISA_REPRO)
FFLAGS_DEBUG = -g -O0 -check -check noarg_temp_created -check nopointer -warn -warn noerrors -fp-stack-check -fstack-protector-all -fpe0 -debug -traceback -ftrapuv $(ISA_DEBUG)
...
CFLAGS_DEBUG = -O0 -g -ftrapuv -traceback $(ISA_DEBUG)
...
LIBS := $(shell pkg-config --libs-only-L netcdf-fortran) $(shell pkg-config --libs-only-L mpich)
#LIBS += -lmpifort -lmpi
